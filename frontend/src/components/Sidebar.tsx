...
import { 
  ...
  Users, Trophy, BookOpen, UserPlus
} from 'lucide-react';
  ...
  return (
    <>
      ...
      <motion.aside 
        ...
        className="hidden md:flex flex-col fixed left-0 top-0 h-screen border-r border-neutral-200/50 dark:border-neutral-800/60 glass p-4 z-30 justify-between select-none"
      >
        <div className="flex flex-col gap-6 w-full relative">
          ...
          <button
            ...
            className="absolute -right-7 top-4 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 text-neutral-500 hover:text-black dark:hover:text-white p-1 rounded-full shadow-md z-40 active-shrink cursor-pointer"
          >
                ...
                <div
                  className={`flex items-center gap-4 px-3.5 py-3 rounded-2xl cursor-pointer transition-all duration-200 relative ${
                    isActive 
                      ...
                      : 'text-neutral-500 dark:text-neutral-450 hover:text-neutral-950 dark:hover:text-white hover:bg-white/5'
                  }`}
                >
        ...
        {/* Footer Actions */}
        <div className="flex flex-col gap-1.5 border-t border-neutral-200/50 dark:border-neutral-800/60 pt-4">
          {/* Theme Switcher */}
          <button
            onClick={toggleTheme}
            className="flex items-center gap-4 px-3.5 py-3 rounded-2xl cursor-pointer text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100/50 dark:hover:bg-neutral-900/30 hover:text-neutral-950 dark:hover:text-white transition-all duration-200 w-full text-left bg-transparent border-0 active-shrink"
          >
          ...
          <div className="flex items-center gap-3 p-2 rounded-2xl bg-neutral-100/50 dark:bg-neutral-900/30 border border-neutral-200/30 dark:border-neutral-800/30 overflow-hidden">
            {user.profile?.avatarUrl ? (
              <img 
                src={user.profile.avatarUrl} 
                alt="avatar" 
                className="w-8 h-8 rounded-xl object-cover flex-shrink-0 border border-neutral-300 dark:border-neutral-700" 
              />
            ) : (
            ...
            {!isCollapsed && (
              ...
              >
                <span className="font-bold text-xs text-neutral-900 dark:text-white truncate">{user.username}</span>
            ...
            {!isCollapsed && (
              <button
                ...
                title="Sign Out"
                className="p-1.5 hover:bg-neutral-200 dark:hover:bg-neutral-800 text-neutral-400 hover:text-red-500 rounded-lg bg-transparent border-0 cursor-pointer active-shrink"
              >
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full h-16 border-t border-white/5 bg-brand-bg/90 backdrop-blur-xl flex items-center justify-around px-4 z-30">
        {menuItems
            ...
            const content = isCreate ? (
              ...
              <div
                className={`p-2.5 rounded-xl transition-all active-shrink relative ${
                  isActive ? 'text-brand-orange scale-110' : 'text-neutral-500 hover:text-neutral-350'
                }`}
              >
